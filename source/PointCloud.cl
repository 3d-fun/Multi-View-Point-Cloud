...
void multMM(Matrix* m1, Matrix* m2, Matrix* result);
void multMV(Matrix* m, Vector* v, Vector* result);
void set(Vector* v, float x, float y, float z, float w);
void getImageIndices(int globalIndex, int* x, int* y, int* imageIndex, int imageWidth, int imageHeight);
void sampleImage(__read_only image3d_t stereoImages, int x, int y, int imageIndex, int sampleWindow, uchar* sample);
void cameraSubset(int centerCamera, int subsetSize, int numCameras, int* subset);
void getLocalDepthRange(Matrix* cameraMatrix, __global float* globalRange, float* depthRange);
bool isClipped(Vector* screenCoord, int imageWidth, int imageHeight);
...
bool isClipped(Vector* screenCoord, int imageWidth, int imageHeight)
{
	if (screenCoord->v[0] >= (float)imageWidth || screenCoord->v[1] < 0.0)
		return true;
	...
	__global int* numValidDepthSamples, __global float* depthSample, __global float* depthCorrelation, int imageReferenceIndex, int rayMarchIndex)
{
	...
	//getLocalDepthRange(&transform, globalSearchRange, depthRange);
	...
	if(sampleSum >= 6*SAMPLE_WINDOW_SIZE*SAMPLE_WINDOW_SIZE)
	{
		...
		for (int localCameraIndex = 0; localCameraIndex < LOCAL_CAMERA_SIZE; localCameraIndex++)
		{
			...
			{
				Matrix transform_j = transformMatrix[localCameraIndex];
		...
		if (acceptedNcc >= 2)
		{
			...
			if(nccSum > corr || numValidDepthSamples[id] == 0)
			{
				depthSample[id] = d;
